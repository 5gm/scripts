...
local currencyTab = Window:CreateTab("currency", 4483362458) -- Title, Image
    ...
    Callback = function(p)
        ...
        if autoplasma then
            local autoplasmaLoop = task.spawn(function()
                while autoplasma do
                    game:GetService("ReplicatedStorage"):WaitForChild("RE"):FireServer("coins")
                    task.wait(0.001)
                end
            end)
        end
